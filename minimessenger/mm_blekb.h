...
// to add more.
// ----------------------------------------------------------------------------
constexpr uint16_t BT_SERVICE_HID_1812 = 0x1812;          // HID over GATT — top-level service exposed by all BLE keyboards /
                                                          // mice / gamepads.
constexpr uint16_t BT_CHAR_HID_REPORT_2A4D = 0x2A4D;      // HID Report — there is ONE characteristic instance per Report ID
                                                          // (input / output / feature).
constexpr uint16_t BT_APPEARANCE_KEYBOARD_03C1 = 0x03C1;  // Generic HID → Keyboard. Optional in adv data but a strong
                                                          // positive signal when present.
constexpr uint16_t BT_APPEARANCE_MOUSE_03C2 = 0x03C2;     // Generic HID → Mouse. Not used to filter today, kept for symmetry
                                                          // / future negative filtering.
 ...
 * (detection, connection+bonding, reconnection, transmitting keystrokes to
 * upper layer)
 *
 * Scan filter is now based on the HID GATT service UUID (BT_SERVICE_HID_1812)
 * instead of the advertised device name. Any keyboard that exposes
 * HID-over-GATT is accepted — regardless of vendor name, model, or whether the
 * firmware advertises a name at all. The trade-off is that any HID peripheral
 * in pairing mode and in range can match (mice, gamepads). Mouse reports are
 * 3–4 bytes and are silently dropped by decodeHIDReport's 8-byte length guard,
 * so an accidental mouse bond is harmless apart from squatting the BLE slot
 * until `cmd bonds` is sent.
 *
 ...
 * by the stack and are not overridden here — the base class no-op defaults are
 * sufficient.
 */
