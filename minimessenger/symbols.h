...
enum DisplayType {
    ...
    OLEDSHIELD,  // lib Adafruit_SSD1306 — alternate target, stub implementation
                 // only
};
...
enum DisplayPowerState {
    DISPLAY_ON,
    DISPLAY_DIMMED,  // 50% backlight (if TFT_BL is a real PWM pin); otherwise no
                     // visible change but timer still progresses to OFF
