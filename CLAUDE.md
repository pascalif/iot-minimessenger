...
- `admin/liveness/<deviceId>` — **one topic per device**, retained, carries every flavour of state transition. Payload = `"<TYPE> <epochSeconds>"` for BOOT / RECO / LIVE, or just `"DEAD"` (no timestamp) for the Will. Each device subscribes to the wildcard `admin/liveness/+` so a fresh boot receives one retained payload per peer in a single shot — including DEAD tombstones for peers whose broker-detected disconnect fired their Will. The `deviceId` is extracted from the topic suffix; the dispatcher in `mqtt.ino` ignores stale retains whose epoch is older than `MQTT_KEEPALIVE_INTERVAL_MS + 5 s` (catches peers that crashed without firing a Will and left a frozen LIVE retained). See `docs/howto_mqtt.md`.
...
- `onReceivedContactOnline(remoteDeviceId, ContactLiveness liveness)` — called by `onMqttIncomingMessage()` (mqtt.ino) for every `admin/liveness/<id>` payload after the dispatcher has parsed the leading TYPE word via `parseMQTTLiveness()` and filtered stale retains. The `liveness` argument is a two-state `enum class` (defined in `contacts.h`): `ContactLiveness::LIVE` (peer present, mapped from MQTT subtypes BOOT/RECO/LIVE) or `ContactLiveness::DEAD` (peer gone, mapped from the DEAD Will payload). The fine-grained wire subtype is `MQTTLiveness` in `mqtt.h`. `id` always comes from the topic suffix; self-echo filtering happens in the dispatcher, not here. `LIVE` allocates or refreshes a slot; `DEAD` releases it immediately. A 6th simultaneous contact is logged and dropped — there is no LRU eviction.
- `contactsTick()` — called every `loop()` iteration; reads `millis()` internally and releases any slot older than `MQTT_KEEPALIVE_INTERVAL_MS + 5 s` (~125 s with the current 120 s keepalive). Covers crashed peers whose Will never fired. Does **not** take the loop-top `currentMillis` as a parameter on purpose: `onReceivedContactOnline()` stamps `lastSeenMs` with its own fresh `millis()`, so passing a stale `currentMillis` here would underflow the unsigned subtraction and immediately evict a contact whose liveness was received earlier in the same iteration.
